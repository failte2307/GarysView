...
public class FailteController {
	...
	@RequestMapping(method =  RequestMethod.POST, produces="application/json", value="createemployee/employeeId/{employeeId}/departmentId/{departmentId}/employeeName/{employeeName}/telephoneNo/{telephoneNo}/email/{email}")
	@ResponseBody
	public void createEmployee(@PathVariable ("employeeId") int employeeId,@PathVariable("departmentId") int departmentId,@PathVariable("employeeName") String employeeName, @PathVariable("telephoneNo")String telephoneNo, @PathVariable("email")String email) {
	EmployeeDTO newemployeeDTO = new EmployeeDTO(employeeId,departmentId,employeeName,telephoneNo,email);		
	...
	//		logger.info("creating new employee with the following params " + query.toString());
			failteService.createEmployee(newemployeeDTO);
